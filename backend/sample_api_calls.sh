...
curl -X POST http://134.33.240.184:8000/query \
...
curl -X POST http://134.33.240.184:8000/conversations/query \
-H "Content-Type: application/json" \
-d '{
  "user_id": "user123",
...
curl -X GET http://134.33.240.184:8000/conversations?user_id=user123
echo -e "\n"

# 4. Get the history of a specific conversation
echo "--- Getting a specific conversation's history ---"
curl -X GET http://134.33.240.184:8000/conversations/your_conversation_id_here
echo -e "\n"

# 5. Delete a conversation
echo "--- Deleting a conversation ---"
curl -X DELETE http://134.33.240.184:8000/conversations/your_conversation_id_here
echo -e "\n"
...
curl -X GET http://134.33.240.184:8000/servers
echo -e "\n"

# 7. List connected servers
echo "--- Listing connected servers ---"
curl -X GET http://134.33.240.184:8000/servers/connected
echo -e "\n"

# 8. Get detailed information for a specific server
echo "--- Getting info for 'zendesk' server ---"
curl -X GET http://134.33.240.184:8000/servers/zendesk
echo -e "\n"
echo "--- Getting info for 'jira' server ---"
curl -X GET http://134.33.240.184:8000/servers/jira
echo -e "\n"
echo "--- Getting info for 'confluence' server ---"
curl -X GET http://134.33.240.184:8000/servers/confluence
echo -e "\n"

# 9. Connect to a server
echo "--- Connecting to 'zendesk' server ---"
curl -X POST http://134.33.240.184:8000/servers/connect \
-H "Content-Type: application/json" \
-d '{
  "server_name": "zendesk"
}'
...
curl -X POST http://134.33.240.184:8000/servers/disconnect \
-H "Content-Type: application/json" \
-d '{
  "server_name": "zendesk"
}'
...
curl -X POST http://134.33.240.184:8000/batch/servers/connect \
-H "Content-Type: application/json" \
-d '{
  "server_names": ["zendesk", "jira", "confluence"],
  "parallel": true
...
curl -X POST http://134.33.240.184:8000/tools/execute \
-H "Content-Type: application/json" \
-d '{
  "server_name": "zendesk",
